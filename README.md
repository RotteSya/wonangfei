...
- 首页：一个核心数字（滑轮式 odometer 实时滚动），熔金色液态进度条、随作息时段变色的环境光晕、长按数字爆金币彩蛋；右上角为分享入口，弹出今日窝囊战报卡片。
...
- 首页工资实时计算、隐私打码、进度条。`OdometerMoneyText`（`WNF/OdometerText.swift`）逐位滑轮滚动 + 满元 `+¥1` 弹片 + 金色流光；进度条是 `wnfMoltenGold` Metal 着色器液态填充；`HomeAmbientBackdrop` 随工作时段变色并缓慢漂移（放在 `.background{}` 里裁剪，绝不撑大页面布局，否则会把 TopBar 按钮挤出屏幕）；长按数字触发 `CoinFountainBurst` 爆金币彩蛋（reduce-motion 下关闭）。视觉/动效层集中在 `WNF/WNFShaders.metal`、`WNF/ShaderFX.swift`、`WNF/OdometerText.swift`、`WNF/PagerShell.swift`。
...
- UI 巡演测试：`WNFUITests/JellyTourUITests.swift` 的 `testGrandTour` 驱动并验证每个交互（长按爆金币、隐私切换、果冻滑页、图表刮取、周期切换、tab 胶囊、分享卡 genie 进出），打印 `TOUR-MARK` 时间标记，可配合 `simctl io recordVideo` 录像。它也是布局护栏：环境光晕回归把 TopBar 按钮挤出屏幕时，这个测试会以「按钮不可见」失败。
