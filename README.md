...
- 首页分享卡片：背景虚化、今日窝囊费/上班时长、卡片内隐藏敏感信息、系统分享和退出；系统分享渲图期间分享按钮会显示 loading 并防重复点击，渲图前会先让出一帧刷新 UI，再用 SwiftUI `ImageRenderer.render(rasterizationScale:)` 输出系统分享图；分享卡组件集中在 `WNF/ShareCard.swift`。
