...
It relies on accessibility identifiers (`home.money`, `home.share`, `home.privacy`, `tab.<home|records|settings>`, `records.period.<week|month|year>`, `records.chart`, `share.cancel`) — keep them when refactoring those controls. The test also acts as a layout guard: it failed (button "off-screen") when the ambient backdrop regressed the TopBar.
...
- `WNF/HomeView.swift` home page: `HomeAmbientBackdrop` (workday-phase-tinted drifting glow + grain) **must stay in `.background{}` with an explicit page-sized frame + `.clipped()`** — it was a ZStack sibling and its 580pt circles expanded the page layout, centering the content and shoving the TopBar buttons off-screen. The progress bar is the `wnfMoltenGold` shader carved out of a full-width rect; long-pressing the money fires a keyframe-driven `CoinFountainBurst` (reduce-motion gated); the existing per-second `HomeCoinDropLayer` and `CoinSourceAnchorKey` are unchanged. Pass `isActive` (`abs(pagerProgress) < 1`) down so the off-screen page pauses its per-second `TimelineView` and shader clocks.
