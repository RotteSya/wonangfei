...
- `WNF/PolishShaders.metal` + `WNF/PolishEffects.swift` own the shared "premium feel" layer: `sheen` / `aurora` / `liquidGold` / `jellyWarp` stitchable shaders and their SwiftUI drivers (`LoopingSheen`, `sheenSweep(on:)`, `AuroraBackdrop`, `LiquidGoldCapsule`, `TabSlideIn`/`TabSlideOut`, `ElasticSegmentedControl`, `CoinPopBurst`, `PulseRing`). Constraints: shader args are not animatable, so animated effects go through `Animatable` modifiers or `TimelineView` time (same pattern as `GenieEmergence`); the tab jelly warp must stay disabled for the home tab because SwiftUI shader effects drop UIKit-backed content (the mascot `AVPlayerLayer`); every looping effect must gate on `accessibilityReduceMotion`.
